import { validAnalysis, normalizeAnalysis, topicAnalysisInstructions, analysisDimensions } from './question-analysis.mjs';
...
export function validAssessment(a) {
  return Boolean(a && ['summary','feasibility','innovation','value'].every(k => text(a[k]) && !/^(待确认|待估|未知|待核查)[。！!]*$/.test(a[k].trim()))
    ...
    && (a.analysis === undefined || validAnalysis(a.analysis))
...
export function normalizeAssessment(a) {
  ...
  return { version: a.version === 2 || a.difficulty ? 2 : 1, origin: 'agent', kind: 'planning_estimate',
    ...
    ...(a.analysis ? {analysis:normalizeAnalysis(a.analysis)} : {}),
...
template 只选 comparison（图表比较）、reading（文字比较）、accordion（原位逐题预览）、brief（价值与个人条件）、landscape（领域问题分布）、timeline（周期与资源取舍）；reason 说明内容为何适合此版式。可按每题选不同模板，不能返回 HTML/CSS、可执行内容或自行省略四项判断。渲染器负责数据图；模型只提供结构化内容。比较表只作快速扫描；每题的判断、依据、资源条件与完整论证归入该题原位浮层，公共周期图不承载个人适合度结论。${topicAnalysisInstructions}`;
