...
function RecordEditor({project,artifactId,item,kind,onProject,onSaved,onDirty,onDiscuss}) {
  ...
  return <div className="continuity-editor" onCompositionStart={()=>{compose.current=true;}} onCompositionEnd={()=>{compose.current=false;setTimeout(save,0);}}>
    ...
    <fieldset disabled={saving||Boolean(pending.current)}><label>记录名称<input aria-label="记录名称" value={draft.title} onChange={e=>change('title',e.target.value)} maxLength={2000}/></label>
      ...
      <div className="continuity-mainfields">{Object.entries(fields).map(([key,label])=><label key={key} className={['text','design'].includes(key)?'continuity-field-wide':undefined}>{label}<textarea aria-label={label} rows={['text','design','analysisPlan'].includes(key)?4:2} value={draft.payload[key]??''} onChange={e=>change(key,e.target.value)} placeholder="尚不清楚可留空；只记录实际已有信息"/>{kind==='protocol'&&item&&<button className="continuity-field-discuss" type="button" disabled={saving} onClick={async()=>{const saved=dirty?await save():{itemId:item.id,revisionId:base};if(saved)onDiscuss(`请围绕方案中的“${label}”解释当前选择是否合适，给出取舍理由：${draft.payload[key]||'尚未填写'}。`,{id:saved.itemId,revisionId:saved.revisionId,text:draft.title,kind:'protocol',fieldKey:key});}}>讨论这项</button>}</label>)}</div>
...
export function ResearchContinuity({project,artifactId,onProject,onClose,onDiscuss,onTarget}) {
  ...
  return <Modal title="方案与研究记录" onClose={onClose}><div className="continuity-workspace">
    ...
    {tab==='practice'&&<div className="continuity-practice"><h3>只记录值得改进的断点</h3><p>在同一研究步骤两边分别产出后比较。额外求助可以继续真实研究，同时记录带回了什么。</p><label>本次路径<select value={path} onChange={e=>setPath(e.target.value)}><option value="workbench">科研工作台</option><option value="gpt">提示词＋GPT 网页版</option><option value="zcode">Z Code</option><option value="external">其他工作方式</option></select></label>{path==='external'&&<label>工作方式名称<input value={pathName} onChange={e=>setPathName(e.target.value)}/></label>}<label>实际模型、提示、材料范围与反馈轮次<textarea value={configuration} onChange={e=>setConfiguration(e.target.value)} placeholder="不知道的内容保留未知"/></label><label>卡在哪里，怎样继续，产生什么影响<textarea aria-label="卡在哪里，怎样继续，产生什么影响" value={note} onChange={e=>setNote(e.target.value)} rows={4}/></label><button disabled={busy||!note.trim()} onClick={async()=>{const r=await command('record-comparison-event',{path,pathName:path==='external'?pathName:undefined,configuration,text:note});if(r)setNote('');}}>保存这次观察</button><button onClick={()=>download('同题实践记录.json',JSON.stringify(project.researchEvents.filter(e=>e.type==='comparison_observation'),null,2))}>导出实践记录</button>{project.researchEvents.filter(e=>e.type==='comparison_observation'&&e.target.scopeId===work.snapshot.scope.id).slice().reverse().map(e=><section key={e.id}><strong>{e.detail.pathName??({gpt:'GPT 网页版',workbench:'科研工作台',zcode:'Z Code'})[e.detail.path]??'外部工作方式'} · {date(e.at)}</strong><p>{e.detail.text}</p></section>)}</div>}
