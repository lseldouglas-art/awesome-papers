...
export function DataFigures({project,artifact,disabled,onTemplateEdit}){
  ...
  return <section className="revision-tool-sheet figure-workspace data-figure-studio">
    ...
    <div className="data-figure-columns"><div className="data-figure-input">
      ...
      {figure?<><img className="scientific-figure-preview" src={url} alt={figure.title}/><label><input type="checkbox" checked={linkCurrentResearch} onChange={e=>setLinkCurrentResearch(e.target.checked)}/>本图实际使用当前采用方案和结果，保存时关联</label><div className="figure-export"><button disabled={disabled} onClick={()=>finish(()=>onTemplateEdit({action:'save-figure',figure:input,linkCurrentResearch}))}>保存图稿版本</button><button onClick={()=>finish(f=>download(`${f.title}.svg`,figureSvg(f),'image/svg+xml'))}>导出矢量 SVG</button><button onClick={()=>finish(f=>png(f))}>导出高清 PNG</button><ExportEvidence projectId={project.id} title={figure.title} refs={artifact.topicWorkspace.figures?.some(f=>f.id===figure.id&&JSON.stringify(f.input)===JSON.stringify(input))?[nestedRef('figure',artifact,null,null,figure.id)]:[]} draft={artifact.topicWorkspace.figures?.some(f=>f.id===figure.id&&JSON.stringify(f.input)===JSON.stringify(input))?null:input} files={[{name:'图稿.svg',content:figureSvg(figure)},{name:'输入数据.json',content:JSON.stringify(input,null,2)}]}/></div></>:<div className="data-preview-empty"><span aria-hidden="true">▥ →</span><h3>在这里预览图稿</h3><p>{input.type==='flow'?'输入步骤后生成流程图。':'粘贴或导入数据后生成图表。'}</p></div>}
