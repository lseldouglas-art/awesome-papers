...
export function TaskActivity({ task, onStop }) {
  const [elapsed, setElapsed] = useState(0);
  useEffect(() => {
    const update = () => setElapsed(Math.max(0, Math.floor((Date.now() - Date.parse(task.createdAt)) / 1000)));
    update(); const timer = setInterval(update, 1000); return () => clearInterval(timer);
  }, [task.id]);
  return <div className="task-status" role="status"><span className="activity-dot"/><div><span>{taskPurposes[task.mode] ?? task.progress}</span><small>{elapsed} 秒 · {elapsed >= 30 && task.mode !== 'retrieve' ? '仍在等待本次回答，可随时停止；已有材料与记录会保留。' : '结果完成后会显示在这里，供你查看与继续。'}</small></div><button onClick={onStop}>停止</button></div>;
}
...
export function SearchReceipt({ search, previous }) {
  ...
  return <div className="search-receipt">
    ...
    <p>{search.searchedAt ? new Date(search.searchedAt).toLocaleString('zh-CN') : '时间未记录'} · 相关性／发表时间各取前 20 条，去重后读取题名与摘要。</p>
...
export function ResearchEntry({ query, onQuery, proposal, search, task, busy, currentResult, configured, configurationPending, recipient, failure, onPrepare, onSearch, onResume, onModel, materialReview, selectedCount }) {
  ...
  const prepare = () => onPrepare('我目前只有一个模糊的研究想法，请承接当前目标和已有反馈，按领域认识的方法构建初步探索范围和 PubMed 检索式。可以先整体了解；不要求补齐问卷，不擅自限定年份、语言或文献类型，也不替我确定题目。');
