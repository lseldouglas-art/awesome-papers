...
import { RetrievalScope } from './RetrievalScope.jsx';
...
export function SearchReceipt({ search, previous }) {
  ...
  return <div className="search-receipt">
    ...
    <p>{search.searchedAt ? new Date(search.searchedAt).toLocaleString('zh-CN') : '时间未记录'} · {search.retrievalOptions ? `${search.dateRange} · 计划获取 ${search.plan?.target ?? '待查询'} 篇题名／摘要。` : `相关性／发表时间各取第 ${(search.offset ?? 0) + 1}–${(search.offset ?? 0) + 20} 条，去重后读取题名与摘要。`}</p>
...
export function TargetedMaterialSearch({ query, onQuery, proposal, search, searches = [], previousQuery = '', previousSearch, scopeMode = 'focused', task, busy, configured, configurationPending, recipient, failure, onPrepare, onSearch, onModel, materialReview, focusText, onLibrary, libraryReady, retrievalOptions, onRetrievalOptions }) {
  ...
  return <details className="scope-details targeted-material-search"><summary>{libraryReady ? '围绕选定专题检索 · 可扩大范围' : '定向检索 · 扩充材料'}</summary>
    <section className="research-entry" aria-label="定向补查材料">
      ...
      {query.trim() && <><RetrievalScope query={query} options={retrievalOptions} onChange={onRetrievalOptions} onSearch={onSearch} search={search} disabled={disabled}/><button disabled={disabled} onClick={()=>prepare()}>重构本题检索式</button></>}
...
export function ResearchEntry({ query, onQuery, proposal, search, task, busy, currentResult, configured, configurationPending, recipient, failure, onPrepare, onSearch, onResume, onModel, materialReview, selectedCount, embedded = false, prepareLabel, prepareDisabled = false, retrievalOptions, onRetrievalOptions }) {
  ...
  const reusable = search?.accessIds.length > 0 && (search.baseQuery ?? search.query).trim() === query.trim();
