...
function Manuscript(props){
  ...
  return <section className="manuscript-workbench" aria-label="正文初稿工作台">
    ...
    {view==='draft'&&<>
      ...
      <div className="manuscript-exports"><select aria-label="整篇初稿版本" value={selected} onChange={e=>setSelected(e.target.value)}><option value="current">当前正文</option>{history.map((m,i)=><option key={m.id} value={m.id}>初稿 {i+1} · {date(m.at)}{m.complete?'':' · 部分'}</option>)}</select><button disabled={!hasText||running} onClick={()=>download(`${manuscript.title}-正文初稿.md`,manuscriptMarkdown(manuscript),'text/markdown')}>导出正文 .md</button><button disabled={!hasText||running} onClick={()=>download(`${manuscript.title}-正文初稿.html`,manuscriptHtml(manuscript),'text/html')}>导出排版稿</button><ExportEvidence projectId={project.id} title={manuscript.title} refs={Object.entries(manuscript.sectionVersions).filter(([,id])=>id).map(([sectionId,id])=>nestedRef('writing',artifact,outline.id,sectionId,id))} files={[{name:`${manuscript.title}.md`,content:manuscriptMarkdown(manuscript)},{name:`${manuscript.title}.html`,content:manuscriptHtml(manuscript)}]} disabled={!hasText||running}/></div>
      ...
      {hasText&&<footer className="manuscript-next"><span>正文导出保持简洁；成果与依据包另附版本、来源和核对边界。</span><button disabled={!ready} onClick={()=>go('template')}>下一步：模板阅读<ArrowRight size={16}/></button></footer>}
    </>}
