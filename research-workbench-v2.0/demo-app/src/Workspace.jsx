import { ResearchPartner } from './ResearchPartner.jsx';
import { pendingDomainAnalysis } from '../../shared/research-guidance.mjs';
...
function ArtifactWorkspace({ onIntake, project, setProject, onProjects, onHome, artifactId, onArtifact }) {
  const pRef = useRef(project); pRef.current = project;
  const artifact = project.artifacts[artifactId];
  const [selectedQuestion, setSelectedQuestion] = useState(() => { let saved;try{saved=JSON.parse(localStorage.getItem(`rw2:partner-target:v1:${project.id}:${artifactId}`));}catch{} const ref = saved?{itemId:saved.id,revisionId:saved.revisionId}:artifact.branchQuestion, item = project.researchItems?.[ref?.itemId], revision = item?.revisions.find(r => r.id === ref.revisionId); return revision ? { id: item.id, revisionId: revision.id, text: revision.text, scope: revision.scope, kind: item.kind, evidence: (revision.evidenceIds ?? []).map(id => project.evidence[id]).filter(Boolean) } : null; });
  useEffect(()=>{try{localStorage.setItem(`rw2:partner-target:v1:${project.id}:${artifactId}`,JSON.stringify(selectedQuestion?{id:selectedQuestion.id,revisionId:selectedQuestion.revisionId}:null));}catch{}},[selectedQuestion?.id,selectedQuestion?.revisionId,project.id,artifactId]);
  ...
  const [retrievalOptions,setRetrievalOptions]=useState(()=>{const saved=(cached.queryOwner===artifactId?cached.retrievalOptions:null)??latestSearch?.retrievalOptions;return saved&&!retrievalScopeError(saved)?saved:setRetrievalRange({type:artifact.kind==='brief'?'reviews':'any'},artifact.kind==='brief'?'recent5':'all');});
  ...
  const persistComposer = () => { try { localStorage.setItem(cacheKey, JSON.stringify({ message: messageValue.current, note, mode, materialIds, replyTask, query, queryBase: scopedQuery(), queryOwner: artifactId, searchScopeMode, previousQuery, topicOptions, topicStep, retrievalOptions, materialScopeIds: artifact.draft.sourceAccessIds })); return true; } catch { setError('浏览器无法保存尚未发送的内容，请先复制保留。'); return false; } };
  ...
  const focusConversation = () => { setDomainComposerOpen(true); setPanel('discussion'); requestAnimationFrame(() => composerRef.current?.focus()); };
  ...
  const activeTask = tasks.find(isRunning), latestTask = tasks.filter(t => t.artifactId === artifactId && !t.input.reviewConstraints).at(-1);
  ...
  useEffect(() => { persistComposer(); }, [message, note, mode, materialIds, query, replyTask, searchScopeMode, topicOptions, topicStep, retrievalOptions, artifact.draft.sourceAccessIds]);
  ...
  const priorMaterialScope = useRef(new Set(cached.materialScopeIds ?? artifact.draft.sourceAccessIds));
  useEffect(() => {
    const added = artifact.draft.sourceAccessIds.filter(id => !priorMaterialScope.current.has(id));
    priorMaterialScope.current = new Set(artifact.draft.sourceAccessIds);
    if (artifact.kind !== 'topic_library' && added.length) { setMaterialIds(ids => uniqueAccessIds(project, [...ids, ...added])); setMaterialChoice(true); }
  }, [artifact.draft.sourceAccessIds]);
  useEffect(() => {
    if (!activeTask) return;
    let stopped = false, timer;
    const poll = async () => { try { const task = await api.request(`/api/projects/${project.id}/research-tasks/${activeTask.id}`); if (stopped) return; if (isRunning(task)) { if((topicTaskModes.includes(task.mode)||(task.mode==='retrieve'&&task.input.retrievalOptions))&&task.progress!==pRef.current.researchTasks[task.id]?.progress)await refresh(); const p = pRef.current; commitProject({ ...p, researchTasks: { ...p.researchTasks, [task.id]: task } }); timer = setTimeout(poll, 1400); } else { const p = await refresh(); await refreshCaps(); await receiveTask(task, p); } } catch (e) { if (!stopped) { setError('暂时无法读取任务状态；请求不会自动重新执行。'); timer = setTimeout(poll, 3000); } } };
  ...
  const startTask = async (kind, text = '', options = {}) => {
    ...
    const rangeRetry=kind==='retrieve'&&options.retryOf&&pRef.current.researchTasks[options.retryOf]?.input.retrievalOptions;
    if(rangeRetry){const prior=pRef.current.researchTasks[options.retryOf];options={...options,query:prior.input.query,retrievalOptions:prior.input.retrievalOptions,...(prior.searchId?{resumeSearchId:prior.searchId}:{})};}
    ...
    const submission = { requestId: crypto.randomUUID(), body: { artifactId, baseVersion: draft.version(), mode: kind,
      ...(text.trim() ? { text: text.trim() } : {}), ...(kind === 'retrieve' ? { retrievalOffset: options.retrievalOffset ?? (options.retryOf ? pRef.current.researchTasks[options.retryOf]?.input.retrievalOffset ?? 0 : 0) } : {}), ...(artifact.kind === 'topic_library' && ['clarify','retrieve'].includes(kind) ? { searchScopeMode: options.searchScopeMode ?? searchScopeMode } : {}), ...(['retrieve', 'landscape','clarify'].includes(kind) ? { query: options.query ?? options.search?.query ?? query, ...(options.search ? { reuseSearchId: options.search.id } : {}) } : {}), accessIds: kind === 'clarify' ? (options.accessIds ?? []) : ['connection', 'retrieve'].includes(kind) ? [] : (options.accessIds ?? materialIds),
      ...(kind==='retrieve'&&(!options.retryOf||rangeRetry)?{retrievalOptions:options.retrievalOptions??retrievalOptions,...(options.resumeSearchId?{resumeSearchId:options.resumeSearchId}:{})}:{}),
      ...(kind==='question-investigation'?{investigationOptions:options.investigationOptions??{search:'saved'}}:{}),
      ...
      ...(options.itemId ? { itemId: options.itemId, itemRevisionId: options.itemRevisionId } : selectedQuestion && kind !== 'compare' ? { itemId: selectedQuestion.id, itemRevisionId: selectedQuestion.revisionId } : {}),
      ...(options.retryOf && !rangeRetry && !topicTaskModes.includes(kind) ? { retryTaskId: options.retryOf } : {}),
    ...
    catch (e) { if (e.status && e.status < 500) { localStorage.removeItem(startKey); setPendingStart(null); } if(e.status===409)await refresh(); throw e; }
    localStorage.removeItem(startKey); setPendingStart(null);
    const p = await refresh(); await receiveTask(p.researchTasks[taskId], p); if(templateModes.includes(kind)){if(kind!=='template-optimize')openTemplates();}else if (['ask','question-investigation'].includes(kind)) setPanel('discussion'); else { setPanel(null); if(!['topic-plan','topic-writing','manuscript-writing'].includes(kind))readRef.current?.scrollTo({ top: 0, behavior: 'smooth' }); } setModal(null); if (isRunning(p.researchTasks[taskId])) setNotice('本步正在进行，已有成果与材料保留。'); return true;
  };
  const postMessage = () => action(async () => {
    ...
    if (mode !== 'record' && isContinueNavigation(message)) {
      ...
      localStorage.setItem(cacheKey, JSON.stringify({ message: '', note, mode, materialIds, replyTask: null, query, queryBase: scopedQuery(), queryOwner: artifactId, searchScopeMode, previousQuery, topicOptions, retrievalOptions, materialScopeIds: artifact.draft.sourceAccessIds }));
  ...
  const matchingSearch = artifactSearches.slice().reverse().find(s => (s.baseQuery??s.query).trim() === query.trim() && s.accessIds.every(id => artifact.draft.sourceAccessIds.includes(id)));
  const analysisFailure = pendingDomainAnalysis(tasks, artifactId);
  ...
  return <div className={`app-shell ${panel==='discussion'&&selectedQuestion?'partner-active':''} ${currentResult?.kind === 'brief' ? `domain-page ${domainComposerOpen || panel === 'discussion' ? 'domain-chat-open' : ''}` : ''} ${artifact.kind==='topic_library'?'topic-library-page':''} guided-reading ${artifact.kind==='topic_library'&&topicStep==='figures'?'figure-active':''} ${readingMode === 'visual' ? 'visual-first' : 'full-reading'} ${left ? 'left-open' : ''} ${panel ? 'right-open' : ''}`}>
    ...
    <main className="workspace-main">
      ...
      <div className="reading-scroll" ref={readRef}><article className={currentResult ? "document has-result" : "document"}>
        ...
        {!historical && project.researcherProfile && <details className="intake-saved-plan"><summary>你的研究画像与起步计划</summary><ol>{project.researcherProfile.plan.steps.map(step=><li key={step.title}><strong>{step.title}</strong><p>{step.text}</p></li>)}</ol>{project.researcherProfile.plan.constraints.map(item=><p key={item}>{item}</p>)}<button className="text-button" onClick={()=>action(async()=>{if(await draft.flush())onIntake();})}>核对或调整我的现状</button></details>}
        {!historical && (!currentResult || currentResult.kind === 'brief') && <ResearchEntry analysisSettings={caps?.models} query={query} onQuery={setQuery} proposal={proposal} search={matchingSearch} task={activeTask} busy={busy || !caps} configurationPending={!caps} currentResult={currentResult} configured={caps?.models.configured} recipient={caps?.models.configured ? new URL(caps.models.baseUrl).hostname : null} failure={entryFailure} onPrepare={text => action(() => startTask('clarify', text))} retrievalOptions={retrievalOptions} onRetrievalOptions={setRetrievalOptions} onSearch={options => action(() => startTask('retrieve', '', options))} onResume={() => action(() => startTask('landscape', '', { search: matchingSearch }))} onContinue={analysisFailure ? ()=>action(()=>startTask(analysisFailure.mode,analysisFailure.input.text,{retryOf:analysisFailure.id,query:analysisFailure.input.query,accessIds:uniqueAccessIds(project,[...analysisFailure.input.materials.map(m=>m.accessId),...(project.searches[analysisFailure.searchId]?.accessIds??[])]),search:project.searches[analysisFailure.searchId]})) : undefined} onModel={() => setModal('model')} selectedCount={materialIds.length} materialReview={matchingSearch && <MaterialReview project={project} ids={uniqueAccessIds(project, artifact.draft.sourceAccessIds)} selectedIds={materialIds} onSelection={setMaterialIds} disabled={busy || Boolean(activeTask)} onSource={source}/>}/>}
        ...
        {!historical && !['brief', 'research_branch', 'topic_library'].includes(currentResult?.kind) && <ResearchJourney project={project} artifact={artifact} numbers={numbers} disabled={busy || Boolean(activeTask)} onCommand={researchCommand} onAnalyze={q => {setSelectedQuestion(q);setReplyTask(null);focusConversation();}} onCompare={() => action(() => startTask('compare', '先比较领域选题的价值、潜在创新与个人可行性，给出有假设和依据的月份区间和 1–5 星执行难度；再附拟议研究设计及反证，不保证发表。'))} onDeepen={deepenQuestion} onContinue={continueSelected} onSource={source} onArtifact={selectArtifact} onFocusQuestion={q => { setSelectedQuestion(q); setReplyTask(null); focusConversation(); }}/>}
    ...
    {panel && <aside className="inspector" id="research-inspector"><div className="panel-heading"><h2>{({ notes: '批注', sources: '参考材料', history: '研究进展', discussion: '课题交流', usage: '本机调用记录' })[panel]}</h2><IconButton name="close" label="收起辅助面板" onClick={() => setPanel(null)}/></div><div className="inspector-body">
      ...
      {panel === 'discussion' && selectedQuestion && <ResearchPartner key={`${selectedQuestion.id}:${selectedQuestion.revisionId}`} project={project} artifact={artifact} target={selectedQuestion} accessIds={materialIds} disabled={busy||historical} activeTask={activeTask} model={caps?.models} onSource={source} onTarget={setSelectedQuestion} onRun={(text,options)=>startTask('question-investigation',text,options)} onStop={async task=>{await api.request(`/api/projects/${project.id}/research-tasks/${task.id}/stop`,{method:'POST',requestId:crypto.randomUUID(),body:{}});await refresh();}} onCommand={async(name,body)=>{const r=await api.command(project.id,name,{artifactId,baseStateVersion:pRef.current.researchState.version,...body});await refresh();return r;}}/>}
      {panel === 'discussion' && !selectedQuestion && <><div className="discussion-guide"><p>原文留在中央，围绕当前内容随时交流。</p><button onClick={focusConversation}>在下方输入框继续交流</button></div>{tasks.slice().reverse().map(t => { const r = t.resultId ? project.researchResults[t.resultId] : null; return <details className="task-card" key={t.id} open={t.id === primaryTaskId}><summary>{date(t.createdAt)} · {statuses[t.status]} · {t.input.text ? t.input.text.slice(0, 24) : '研究任务'}</summary><small>{date(t.createdAt)} · {statuses[t.status]}</small>{t.input.text && <blockquote className="task-question">{t.input.text}</blockquote>}<p>{t.input.reviewConstraints?'本次记录与材料已保留，可回到领域认识继续分析。':t.error || t.progress}</p>{t.staleInput && <p className="muted">生成期间你的记录已有更新；这次结果基于较早输入。</p>}{t.proposal && <><p>{t.proposal.explanation}</p><ul>{t.proposal.questions.map(q => <li key={q}>{q}</li>)}</ul><p>{t.proposal.scope}</p></>}{['questions', 'research_brief', 'research_branch','topic_review'].includes(r?.kind) && <button onClick={() => selectArtifact(r.artifactId)}>打开{r.kind === 'questions' ? '候选比较' : r.kind==='topic_review'?'专题文献分析':'课题工作简报'}</button>}{r?.kind === 'answer' && <ResultBlocks numbers={numbers} blocks={r.blocks} citations={r.citations} selected="" onSelect={() => {}} onSource={source} prefix={`answer-${r.id}-`}/>} {r?.kind === 'brief' && <button onClick={() => setPreviewResult(r.id)}>查看领域简报</button>}{!t.input.reviewConstraints && ['failed', 'interrupted', 'cancelled'].includes(t.status) && <button disabled={busy || Boolean(activeTask)} onClick={() => action(() => startTask(t.mode, t.input.text, { retryOf: t.id, search: t.mode === 'landscape' && project.searches[t.searchId]?.accessIds.length ? project.searches[t.searchId] : null }))}>{t.mode === 'landscape' && project.searches[t.searchId]?.accessIds.length ? '用这次已取得的材料继续整理' : '按当前输入重新运行'}</button>}{t.status === 'failed' && !t.input.reviewConstraints && !templateModes.includes(t.mode) && !['topic-writing','manuscript-writing','question-investigation'].includes(t.mode) && t.calls.length > 0 && <div className="saved-output-recheck"><button disabled={busy || Boolean(activeTask)} onClick={() => revalidateTask(t)}>重新校验已保存输出</button><small>保留原始输出和额外说明；只核对完整结果，不再调用模型。</small></div>}{t.status === 'completed' && <button onClick={() => replyTo(t)}>接着这一步交流</button>}<small>模型调用 {t.calls.length} 次 · 费用{t.cost?.status === 'unknown' ? '未知' : t.cost?.amount ?? '未知'}</small></details>; })}{Object.values(project.messages).filter(m => !tasks.some(t => t.messageId === m.id)).slice().reverse().map(m => <div className="message" key={m.id}><small>你的记录 · {date(m.createdAt)}</small><p>{m.text}</p>{m.target && <button className="text-button" onClick={() => openTarget(m.target)}>查看当时原文</button>}</div>)}</>}
    ...
    {modal === 'project' && <ProjectForm onIntake={()=>action(async()=>{if(await draft.flush()){setModal(null);onIntake();}})} project={project} busy={busy} error={error} onClose={() => setModal(null)} onSubmit={body => action(async () => { if (!(await draft.flush())) return; await api.command(project.id, 'update-project', body); await refresh(); setModal(null); })}/>}
    {continuityOpen && <ResearchContinuity project={project} artifactId={artifactId} onProject={commitProject} onClose={()=>setContinuityOpen(false)} onDiscuss={(text,target)=>{if(target)setSelectedQuestion(target);setMessage(text);focusConversation();}} onTarget={target=>{if(target.type==='research_item'){setContinuityOpen(true);return;}if(target.artifactId&&target.artifactId!==artifactId){selectArtifact(target.artifactId);return;}if(target.type==='writing'){changeTopicStep('write');editLibrary({action:'writing-select',outlineId:target.outlineId,sectionId:target.sectionId});}else if(target.type==='figure')changeTopicStep('figures');else if(target.type==='outline')changeTopicStep('outline');else if(target.revisionId)openTarget({artifactId:target.artifactId??target.id,revisionId:target.revisionId});}}/>}
