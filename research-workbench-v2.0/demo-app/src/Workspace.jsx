...
import { ResearchEntry, TargetedMaterialSearch, TaskActivity } from './ResearchEntry.jsx';
...
import { uniqueAccessIds, latestSavedQuery, referenceNumbers } from '../../shared/material-scope.mjs';
...
function ArtifactWorkspace({ project, setProject, onProjects, onHome, artifactId, onArtifact }) {
  ...
  const scopedQuery = () => artifact.kind === 'topic_library' ? (artifact.topicWorkspace?.savedPlan?.query ?? topicQueryState(pRef.current, artifact).query) : latestSavedQuery(pRef.current);
  ...
  const [readingMode, setReadingMode] = useState(() => localStorage.getItem('rw2:reading-mode') ?? 'visual');
  ...
  const focusConversation = () => { setPanel(window.innerWidth < 1100 ? null : 'discussion'); composerRef.current?.focus(); };
  ...
  const activeProposal = artifact.kind === 'topic_library' ? topicQueryState(project, artifact).proposal : project.researchProposal;
  ...
  useEffect(() => { if (artifact.kind !== 'topic_library' && latestSearch && latestSearch.accessIds.length > 0 && latestSearch.id !== priorSearch.current && latestSearch.accessIds.every(id => artifact.draft.sourceAccessIds.includes(id))) { setMaterialIds(uniqueAccessIds(project, latestSearch.accessIds)); setMaterialChoice(true); } priorSearch.current = latestSearch?.id; }, [latestSearch?.id]);
  useEffect(() => {
  ...
  const jump = id => { setSelected(id);if(!historical&&id.startsWith('note-')){setPanel('notes');setNotesTab('records');if(window.innerWidth<1100)setLeft(false);return;} const el = document.getElementById(`block-${id}`); let detail = el?.closest('details'); while (detail) { detail.open = true; detail = detail.parentElement?.closest('details'); } el?.scrollIntoView({ behavior: 'smooth', block: 'center' }); if (window.innerWidth < 1100) setLeft(false); };
  ...
  return <div className={`app-shell ${artifact.kind==='topic_library'?'topic-library-page':''} guided-reading ${artifact.kind==='topic_library'&&topicStep==='figures'?'figure-active':''} ${readingMode === 'visual' ? 'visual-first' : 'full-reading'} ${left ? 'left-open' : ''} ${panel ? 'right-open' : ''}`}>
    ...
    <header className="topbar"><button className="project-title" onClick={() => navigate(() => setModal('project'))}><Icon name="folder" size={22}/><span>{project.name}</span></button><div className="top-actions"><BuildLabel/><span className={`save-state ${draft.status}`} aria-live="polite">{saveLabels[draft.status]}</span><button aria-expanded={left} className={left ? 'active' : ''} onClick={() => setLeft(!left)}><Icon name="outline"/>大纲</button><button aria-expanded={panel === 'notes'} className={panel === 'notes' ? 'active' : ''} onClick={() => setPanel(panel === 'notes' ? null : 'notes')}><Icon name="comment"/>批注</button><button aria-controls="research-inspector" aria-expanded={panel === 'discussion'} className={panel === 'discussion' ? 'active' : ''} onClick={() => setPanel(panel === 'discussion' ? null : 'discussion')}><Icon name="comment"/>交流</button></div></header>
    ...
    <main className="workspace-main">
      ...
      {activeTask && <TaskActivity task={activeTask} onStop={() => action(async () => { await api.request(`/api/projects/${project.id}/research-tasks/${activeTask.id}/stop`, { method: 'POST', body: {} }); await refresh(); })}/>}
      ...
      <div className="reading-scroll" ref={readRef}><article className={currentResult ? "document has-result" : "document"}>
        ...
        {!historical && (!currentResult || currentResult.kind === 'brief') && <ResearchEntry query={query} onQuery={setQuery} proposal={proposal} search={matchingSearch} task={activeTask} busy={busy || !caps} configurationPending={!caps} currentResult={currentResult} configured={caps?.models.configured} recipient={caps?.models.configured ? new URL(caps.models.baseUrl).hostname : null} failure={entryFailure} onPrepare={text => action(() => startTask('clarify', text))} onSearch={() => action(() => startTask('retrieve'))} onResume={() => action(() => startTask('landscape', '', { search: matchingSearch }))} onModel={() => setModal('model')} selectedCount={materialIds.length} materialReview={matchingSearch && <MaterialReview project={project} ids={uniqueAccessIds(project, matchingSearch.accessIds)} selectedIds={materialIds} onSelection={setMaterialIds} disabled={busy || Boolean(activeTask)} onSource={source}/>}/>}
        ...
        {!historical && <ResearchVisuals project={project} artifact={artifact} onOpen={selectArtifact} result={currentResult} numbers={numbers} onJump={jump} onSource={source} onDeepen={deepenQuestion} disabled={busy || Boolean(activeTask)} onCompare={() => action(() => startTask('compare', '依据所选材料先比较领域选题的可行性、潜在创新、投入价值、月份周期与星级难度，再提供具体研究设计与核查路径。'))}/>}
